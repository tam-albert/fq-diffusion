...
def get_model_quantized(args, config_name):
    ...
    return (
        ...
        functools.partial(
            ...
            trans=args.trans if "qk" in args.online_trans else "none",
        ),
...
def get_model_fp16(config_name):
    model = modeling_llama.FlatQuantFP16LlamaForCausalLM.from_pretrained(
        config_name, torch_dtype=torch.float16, attn_implementation="flash_attention_2"
    )
    return (
        model,
        functools.partial(
...
def run_prefill(layer, cache_builder, bsz, prefill_length, hidden_size):
    ...
    test_input = torch.rand(
        (bsz, prefill_length, hidden_size), dtype=torch.float16, device=device
    )
    ...
    def _prefill_and_decode_for_multiple_steps():
        ...
        for i in range(decode_steps):
            layer(
                ...
                past_key_value=past_key_values,
                position_ids=torch.tensor(
                    [[prefill_length + i]] * bsz, device=device, dtype=torch.int32
                ),
            )
...
def run_all_for_model(layer, cache_builder, bsz, prefill, decode, hidden_size):
    ...
    if decode is not None and decode != 0:
        time_decode, memory_decode = run_decode(
            layer, cache_builder, bsz, prefill, decode, hidden_size
        )
        _cleanup()
...
def print_e2e_time(
    ...
    time_decode_f16,
    time_e2e_f16,
    time_prefill_i4_benchmark=None,
    time_decode_i4_benchmark=None,
    time_e2e_i4_benchmark=None,
):
    prefill_speedup = np.mean(time_prefill_f16) / np.mean(time_prefill_i4)
    prefill_benchmark_speedup = (
        np.mean(time_prefill_f16) / np.mean(time_prefill_i4_benchmark)
        if time_prefill_i4_benchmark is not None
        else None
    )
    print(
        f"Prefill time: {np.mean(time_prefill_i4):.3f} +- {1.96 * np.std(time_prefill_i4):.3f}ms\n"
        f"Speedup: {prefill_speedup:.3f}"
        + (
            f" Speedup loss: {prefill_benchmark_speedup - prefill_speedup:.3f}"
            if time_prefill_i4_benchmark is not None
            else ""
        )
    )
    if args.decode_steps is not None and args.decode_steps != 0:
        ...
        decode_benchmark_speedup = (
            np.mean(time_decode_f16) / np.mean(time_decode_i4_benchmark)
            if time_decode_i4_benchmark is not None
            else None
        )
        print(
            f"Decode time: {np.mean(time_decode_i4):.3f} +- {1.96 * np.std(time_decode_i4):.3f}ms\n"
            f"Speedup: {decode_speedup:.3f}"
            + (
                f" Speedup loss: {decode_benchmark_speedup - decode_speedup:.3f}"
                if time_decode_i4_benchmark is not None
                else ""
            )
        )
        e2e_speedup = np.mean(time_e2e_f16) / np.mean(time_e2e_i4)
        e2e_benchmark_speedup = (
            np.mean(time_e2e_f16) / np.mean(time_e2e_i4_benchmark)
            if time_e2e_i4_benchmark is not None
            else None
        )
        print(
            f"E2E time: {np.mean(time_e2e_i4):.3f} +- {1.96 * np.std(time_e2e_i4):.3f}ms\n"
            f"Speedup: {e2e_speedup:.3f}"
            + (
                f" Speedup loss: {e2e_benchmark_speedup - e2e_speedup:.3f}"
                if time_e2e_i4_benchmark is not None
                else ""
            )
        )
...
def benchmark(args):
    for config_name in model_configs:
        ...
        print(f"------------------------- FP16 ------------------------")
        print(
            f"Prefill time: {np.mean(time_prefill_f16):.3f} +- {1.96 * np.std(time_prefill_f16):.3f}ms"
        )
        if args.decode_steps is not None and args.decode_steps != 0:
            print(
                f"Decode time: {np.mean(time_decode_f16):.3f} +- {1.96 * np.std(time_decode_f16):.3f}ms"
            )
            print(
                f"E2E time: {np.mean(time_e2e_f16):.3f} +- {1.96 * np.std(time_e2e_f16):.3f}ms"
            )
        ...
        _cleanup()
        (
            time_prefill_i4_benchmark,
            time_decode_i4_benchmark,
            time_e2e_i4_benchmark,
            mem_i4,
        ) = run_all_for_model(
            layer,
            cache_builder,
        ...
        online_trans_list = [{"o_proj", "qk", "down_proj"}]
        for online_trans in online_trans_list:
            print(
                f"------------------------- Int4 QuaRot ({'+'.join(online_trans)}) ------------------------"
            )
        ...
        online_trans_list = [{"qk", "o_proj", "down_proj", "qkv_proj", "up_gate_proj"}]
        for online_trans in online_trans_list:
            print(
                f"------------------------- Int4 FlatQuant ({'+'.join(online_trans)}) ------------------------"
            )
            args.online_trans = online_trans
            model, cache_builder, hidden_size = get_model_quantized(args, config_name)
            layer = model.model.layers[0]
