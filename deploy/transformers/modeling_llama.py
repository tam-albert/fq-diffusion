...
class FlatQuantFP16LlamaAttention(LlamaFlashAttention2):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if (
            isinstance(self.o_proj_trans, deploy.nn.OnlineTrans)
            and self.o_proj_trans.trans == "matmul"
        ):
            ...
            attn_output.quantized_x = attn_output.quantized_x.reshape(
                bsz, q_len, -1
            ).contiguous()
        else:
            attn_output = self.o_proj_trans(attn_output.transpose(-1, -2)).transpose(
                -1, -2
            )
    ...
    def __init__(self, options, *args, **kwargs):
        ...
        if "o_proj" in self.options.online_trans:
            self.o_proj_trans = deploy.nn.OnlineTrans(
                self.num_heads, trans=options.trans, decompose=False
            )
        self.o_proj = torch.nn.Sequential(
            deploy.nn.Quantizer(), deploy.nn.Linear4bit.from_float(self.o_proj)
        )
        if "qkv_proj" in self.options.online_trans:
            if not self.options.fuseLN:
                self.inp_trans = deploy.nn.OnlineTrans(
                    self.hidden_size, trans=options.trans
                )
...
class FlatQuantLlamaMLP(LlamaMLP):
    def __init__(self, options, *args, **kwargs):
        ...
        if "down_proj" in self.options.online_trans:
            ...
            self.down_proj = torch.nn.Sequential(
                deploy.nn.Quantizer(), deploy.nn.Linear4bit.from_float(self.down_proj)
            )
...
class FlatQuantFP16LlamaForCausalLM(LlamaForCausalLM):
    def __init__(self, config, args=None):
        ...
        for layer_idx, layer in enumerate(self.model.layers):
            layer.self_attn = FlatQuantFP16LlamaAttention(
    ...
    def _get_logits_processor(self, generation_config, *args, **kwargs):
        ...
        # method is called.
        self._expected_max_length = (
            generation_config.max_length
        )  # This value will be reset at the next forward call
...
class FlatQuantLlamaForCausalLM(FlatQuantFP16LlamaForCausalLM):
    def __init__(self, args, config):
        ...
        for layer_idx, layer in enumerate(self.model.layers):
            layer.self_attn = FlatQuantLlamaAttention(
                options=args, config=config, layer_idx=layer_idx
            )
            if args.fuseLN:
                layer.input_layernorm = deploy.nn.RMSNorm(
                    config.hidden_size, eps=config.rms_norm_eps
                )
                layer.post_attention_layernorm = deploy.nn.RMSNorm(
                    config.hidden_size, eps=config.rms_norm_eps
                )
