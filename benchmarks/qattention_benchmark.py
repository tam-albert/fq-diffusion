...
def quantized_kv_cache_decode(
    ...
    query_states = torch.rand(
        (batch_size, 1, num_heads, head_dim), device=device, dtype=torch.float16
    )
    ...
    def _fake_prefill_and_decode():
        ...
        forward_func = cache.update(
            key_states, value_states, layer_idx=0, cache_kwargs={}
        )
...
def qattention_benchmark(args):
    for n_layers, num_heads, head_dim in model_sizes:
        ...
        print(
            f"FP16 time: {np.mean(time_fp16):.3f} +- {1.96 * np.std(time_fp16):.3f}ms"
        )
        print(
            f"Int4 time: {np.mean(time_int4):.3f} +- {1.96 * np.std(time_int4):.3f}ms "
            f"Speedup: {np.mean(time_fp16) / np.mean(time_int4):.3f}"
        )
        print(
            f"Int4 (+had) time: {np.mean(time_int4_had):.3f} +- {1.96 * np.std(time_int4_had):.3f}ms "
            f"Speedup: {np.mean(time_fp16) / np.mean(time_int4_had):.3f}"
        )
        print(
            f"Int4 (+inv) time: {np.mean(time_int4_inv):.3f} +- {1.96 * np.std(time_int4_inv):.3f}ms "
