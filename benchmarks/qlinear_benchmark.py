...
def linear4bit_benchmark(args):
    ...
    for feature_dim_in, feature_dim_out in mlp_sizes:
        for dtype in benchmark_dtypes:
            x = torch.rand((bsz, seq_len, feature_dim_in)).cuda().to(dtype)
            baseline_mod = (
                torch.nn.Linear(feature_dim_in, feature_dim_out, bias=False)
                .cuda()
                .to(dtype)
            )
            ...
            print(
                f"FP16 time: {np.mean(times_baseline):.3f} +- {1.96 * np.std(times_baseline):.3f}ms"
            )
            # int4
            times = []
            for i in range(10):
            ...
            print(
                f"Int4 time: {np.mean(times):.3f} +- {1.96 * np.std(times):.3f}ms\n"
            ...
            print(
                f"Int4 (+had) time: {np.mean(times):.3f} +- {1.96 * np.std(times):.3f}ms\n"
                f"Speedup: {np.mean(times_baseline) / np.mean(times):.3f}x"
            )
            # int4_inv
            ...
            print(
                f"Int4 (+inv, {decompose_size}x{decompose_size} + {feature_dim_in // decompose_size}x{feature_dim_in // decompose_size}) time: {np.mean(times):.3f} +- {1.96 * np.std(times):.3f}ms\n"
                f"Speedup: {np.mean(times_baseline) / np.mean(times):.3f}x"
            )
