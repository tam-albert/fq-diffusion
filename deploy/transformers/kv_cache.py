...
def unpack_i4_and_asym_dequantize(q, scale, zero):
    ...
    q = torch.stack((q & 0x0F, (q >> 4) & 0x0F), dim=-1).view(
...
def batch_decode_i4(
    o, q, kv_data, kv_param, kv_indptr, kv_indices, last_page_offset, layer_idx
):
...
class _AttentionStub(object):
    def __init__(
        self, cache_page_size, device, n_layers, disable_quant, trans_dtype, head_dim
    ):
    ...
    def forward(self, q, num_kv_heads, attention_kwargs, layer_idx):
        ...
        batch_decode(attn_output, q, **attention_kwargs, layer_idx=layer_idx)
...
class MultiLayerPagedKVCache4Bit(Cache):
    def __init__(
        ...
        self.pages = torch.empty(
            ...
            dtype=torch.float16 if disable_quant else torch.uint8,
            device=device,
        )
        self.trans = trans
        if self.trans == "had":
            ...
            self.head_dim = (
                torch.randn([head_dim, head_dim], requires_grad=False)
                .to(trans_dtype)
                .to(device)
            )
        else:
        ...
        self.scales = torch.empty(
            (max_page_cnt * batch_size, n_layers, 2, num_heads, page_size, 2),
            dtype=torch.float16,
            device=device,
        )
        self.page_size = page_size
    ...
    def update(
        ...
        if self.trans_dtype is not None:
            if self.head_dim is None:
                ...
                key_states = torch.matmul(
                    key_states.to(self.trans_dtype), self.head_dim
                )
        ...
        if self._needs_init[layer_idx]:
            ...
            if attention_mask is not None:
                nonzero_indices = (
                    torch.nonzero(attention_mask.flatten(), as_tuple=False)
                    .flatten()
                    .view(-1, 1)
                )
                key_states = key_states.view(
                    self.batch_size * added_length, num_heads * quantized_head_dim
                )
                ...
                key_states = torch.gather(
                    key_states,
                    0,
                    nonzero_indices.expand(-1, num_heads * quantized_head_dim),
                )
                ...
                k_param = torch.gather(
                    k_param, 0, nonzero_indices.expand(-1, num_heads * 2)
                )
                v_param = torch.gather(
                    v_param, 0, nonzero_indices.expand(-1, num_heads * 2)
                )

                seqlens_in_batch = torch.nn.functional.pad(
                    torch.cumsum(
    ...
    def get_cache_specs_for_flash_infer(self, attention_mask):
        if attention_mask is not None:
            ...
            seqlens_in_batch = torch.tensor(
                [self.length], dtype=torch.int32, device=self.device
            ).expand(self.batch_size)
        page_cnt = self.page_cnt_from_length(seqlens_in_batch)
        if (page_cnt[0] != page_cnt).any():
            raise NotImplementedError(
                "Current implementation does not support the case where batches have different number of pages"
            )
        page_cnt = page_cnt[0]
        page_ptr = seqlens_in_batch % self.page_size
        page_ptr = torch.where(
            (seqlens_in_batch != 0) & (page_ptr == 0), self.page_size, page_ptr
        )
        return {
            f"kv_data": self.pages,
            f"kv_indptr": torch.arange(
                0, self.batch_size + 1, device=self.device, dtype=torch.int
            )
            * page_cnt,
            f"kv_indices": (
                (
                    torch.arange(page_cnt, device=self.device, dtype=torch.int)
                    * self.batch_size
                ).unsqueeze(0)
                + torch.arange(
                    self.batch_size, device=self.device, dtype=torch.int
                ).unsqueeze(1)
            ).view(-1),
