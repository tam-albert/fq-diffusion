...
def matmul_kernel(
    ...
    b_ptrs = (
        b_ptr
        + batch_id * stride_bb.to(tl.int64)
    ...
    if is_split:
        ...
        res_ptrs = (
            res_ptr
            + stride_resb.to(tl.int64) * batch_id
            + stride_resm * offs_resm[:, None]
...
def quant_kernel(
    ...
    src_ptrs = (
        src_ptr
        + batch_id * stride_srcb.to(tl.int64)
        + index_rows[:, None] * stride_srcm
        + index_cols[None, :] * stride_srcn
    )
    ...
    dst_ptrs = (
        dst_ptr
        + stride_dstb.to(tl.int64) * batch_id
        + stride_dstm * offs_resm[:, None]
...
def matmul(a, b, c, seq_len):
    ...
    if is_split:
        ...
        packed_tensor = deploy.PackedQuantizedTensor(
            quant_res.reshape(B, -1), output_scale
        )
    else:
...
def benchmark(B, M, N, S, provider):
    ...
    if provider == "cublas":
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: quantizer(torch.matmul(torch.matmul(a, b), c).view(B, -1)),
            quantiles=quantiles,
        )
    if provider == "triton":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul(a, b, c, S), quantiles=quantiles
        )
...
if __name__ == "__main__":
    ...
    for M, N in zip(M_list, N_list):
        ...
        for bs in bs_list:
            # decode
            seq_len = 1
            _, _, _, decode_triton_time_avg, _, _ = benchmark(
                bs * seq_len, M, N, seq_len, "triton"
            )
            _, _, _, decode_cublas_time_avg, _, _ = benchmark(
                bs * seq_len, M, N, seq_len, "cublas"
            )
            # prefill
            seq_len = 2048
            _, _, _, prefill_triton_time_avg, _, _ = benchmark(
                bs * seq_len, M, N, seq_len, "triton"
            )
            _, _, _, prefill_cublas_time_avg, _, _ = benchmark(
                bs * seq_len, M, N, seq_len, "cublas"
            )
            print(
                f" & {bs} & {prefill_cublas_time_avg:.4f} & {decode_cublas_time_avg:.4f} & {prefill_triton_time_avg:.4f} & {decode_triton_time_avg:.4f} & {(prefill_cublas_time_avg / prefill_triton_time_avg):.2f}x & {(decode_cublas_time_avg / decode_triton_time_avg):.2f}x \\\\"
            )
