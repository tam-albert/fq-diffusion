...
def matmul_quant_kernel(
    ...
    b_ptrs = (
        ...
        + (offs_bm[:, None] * stride_bk + offs_k[None, :] * stride_bn)
    )
...
def benchmark(B, M, N, S, provider):
    ...
    if provider == "cublas":
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: quantizer(torch.matmul(b, c).view(B, -1)), quantiles=quantiles
        )
    if provider == "triton":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: block_matmul(b, c, S), quantiles=quantiles
        )
