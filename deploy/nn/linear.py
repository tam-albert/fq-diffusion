...
class Linear4bit(torch.nn.Module):
    def __init__(self, in_features, out_features, bias=False, dtype=torch.float16):
        ...
        self.register_buffer(
            "weight_scales", torch.zeros((self.out_features, 1), requires_grad=False)
        )
        self.register_buffer(
            "weight",
            (
    ...
    def from_float(
        ...
        if weight_scales is not None:
            assert weight_scales.shape == (
                module.out_features,
                1,
            ), "weight_scales should have shape (out_features, 1)"
            ...
            int_module.weight.copy_(
                deploy.functional.pack_i4(int_rounded_weight.to(torch.int8)).cpu()
            )
