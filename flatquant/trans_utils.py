...
class SVDDecomposeTransMatrix(nn.Module):
    def __init__(self, left_size, right_size, add_diag=False, diag_init_para=None):
        ...
        self.linear_diag_left = torch.nn.Parameter(
            torch.ones(left_size), requires_grad=True
        )
        ...
        self.linear_diag_right = torch.nn.Parameter(
            torch.ones(right_size), requires_grad=True
        )
        ...
        if self.add_diag:
            if diag_init_para is None:
                self.diag_scale = torch.nn.Parameter(
                    torch.ones((left_size * right_size)), requires_grad=True
                )
            else:
    ...
    def forward(self, inp, inv_t=False):
        ...
        matrix_left, matrix_right = (
            matrix_u_left @ torch.diag(linear_diag_left) @ matrix_v_left.t(),
...
class InvDecomposeTransMatrix(nn.Module):
    ...
    def forward(self, inp, inv_t=False):
        ...
        if not self._eval_mode:
            matrix_left, matrix_right = (
                self.linear_left.weight,
                self.linear_right.weight,
            )
            if inv_t:
                matrix_left, matrix_right = (
                    get_inverse(matrix_left).T,
                    get_inverse(matrix_right).T,
                )
        else:
            matrix_left, matrix_right = self.matrix_left, self.matrix_right
            if inv_t:
    ...
    def to_eval_mode(self):
        if not self._eval_mode:
            self.matrix_left = nn.Parameter(
                self.linear_left.weight, requires_grad=False
            )
            self.matrix_right = nn.Parameter(
                self.linear_right.weight, requires_grad=False
            )
            self.matrix_left_inv = nn.Parameter(
                get_inverse(self.linear_left.weight).T, requires_grad=False
            )
            self.matrix_right_inv = nn.Parameter(
                get_inverse(self.linear_right.weight).T, requires_grad=False
            )
